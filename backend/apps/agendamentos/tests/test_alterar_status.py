...
@pytest.mark.django_db
def test_status_final_nao_pode_ser_alterado(api_client, atendimento, status_final):
    # Leva o atendimento ate um status final...
    api_client.patch(f"/api/atendimentos/{atendimento.id}/status/", {"status": status_final})

    # ...e tenta mudar de novo, para qualquer outro status.
    outro_status = (
        StatusAtendimento.PENDENTE
        if status_final != StatusAtendimento.PENDENTE
        else StatusAtendimento.REALIZADO
    )
    resposta = api_client.patch(
